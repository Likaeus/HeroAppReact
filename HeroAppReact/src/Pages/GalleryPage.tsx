...
const GalleryPage = () => {
  ...
  useEffect(() => {
    if (paused || interacting || characters.length < 2 || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    let frameId = 0;
    ...
    const advance = (time: number) => {
      ...
      previousTime = time;
      gallery.scrollLeft += elapsed * .022;
  ...
  }, [characters.length, paused, interacting]);
  ...
  return <m.section className="gallery-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    ...
    {!loading && !error && <div className="gallery-track" ref={track} onMouseEnter={() => setInteracting(true)} onMouseLeave={() => setInteracting(false)} onFocusCapture={() => setInteracting(true)} onBlurCapture={() => setInteracting(false)}>
